...
class A(object):
    def __init__(self, level=None):
        if level is None:
            ...
            self.logger = mrglog.get_logger('A class logger', module=True, verbose_lvl=level)


class B(object):
    def __init__(self, level=0):
        self.logger = mrglog.USMLog.get_module_logger('B class logger', verbose_lvl=level)


def test_it(logger, a_logger, b_logger):
    print('logger - verbose level {}\nA class logger - verbose level {}\nB class logger - verbose level {}'.format(
        logger.log_lvl,
        a_logger.log_lvl,
        b_logger.log_lvl,
        ))
